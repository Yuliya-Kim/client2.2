<template>
  <v-container fluid>
    ...
    <v-row>
      <v-col cols="12">
        <h2 class="text-h5 greyDarken-4--text">{{currentComp.type}} {{currentComp.name}}</h2>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12">
        <v-card flat class="pa-5 rounded-lg">
          <v-card-title class="text-h6 font-weight-regular greyDarken-4--text">Дружественные аналоги / недружественные, которые можно заменить текущим</v-card-title>
        </v-card>
      </v-col>
    </v-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setComponent() {
        const currentProject = this.components.find(project => project.name.replaceAll(' ', '_') === this.$route.params.id)
        this.currentComp = currentProject
